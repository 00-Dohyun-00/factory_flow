...
export class ApiService {
  ...
  static async createProductionResult(
    result: ProductionResult,
  ): Promise<ProductionResult> {
    const res = await fetch(
      `${API_URL}/api/production-results`,
      json("POST", result),
    );
    return res.json();
  }
  static async updateProductionResult(
    id: string,
    result: ProductionResult,
  ): Promise<ProductionResult> {
    const res = await fetch(
      `${API_URL}/api/production-results/${id}`,
      json("PUT", result),
    );
    return res.json();
  }
  ...
  // Dashboard Charts
  static async getDailyProduction(): Promise<
    { date: string; quantity: number }[]
  > {
    const res = await get(`${API_URL}/api/dashboard/daily-production`);
    return res.json();
  }
  static async getEquipmentStatus(): Promise<Record<string, number>> {
    const res = await get(`${API_URL}/api/dashboard/equipment-status`);
    return res.json();
  }
  static async getYieldTrend(): Promise<{ date: string; yieldRate: number }[]> {
    const res = await get(`${API_URL}/api/dashboard/yield-trend`);
    return res.json();
  }
  static async getWorkOrderStatus(): Promise<Record<string, number>> {
    const res = await get(`${API_URL}/api/dashboard/workorder-status`);
    return res.json();
  }
  ...
  static async getRecentProductionResults(
    limit: number = 5,
  ): Promise<ProductionResult[]> {
    const res = await get(`${API_URL}/api/production-results`);
  ...
  static async getLowStockMaterials(): Promise<Material[]> {
    ...
    return materials.filter(
      (material) => material.status === "low" || material.status === "critical",
    );
  }
}
